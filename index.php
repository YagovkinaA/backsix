...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  if (!$hasErrors && !empty($_COOKIE[session_name()]) && !empty($_SESSION['login'])) {
    ...
    $user = 'root';
    $pass = 'root';
    $db = new PDO('mysql:host=localhost;dbname=baza4', $user, $pass, array(PDO::ATTR_PERSISTENT => true));
...
// Иначе, если запрос был методом POST, т.е. нужно проверить данные и сохранить их в XML-файл.
  ...
  if (!empty($_COOKIE[session_name()]) &&
    ...
    $user = 'root';
    $pass_db = 'root';
    $db = new PDO('mysql:host=localhost;dbname=baza4', $user, $pass_db, array(PDO::ATTR_PERSISTENT => true));
